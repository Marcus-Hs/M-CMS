...
CKEDITOR.dialog.add( 'link', function( editor ) {
	...
	var javascriptProtocolRegex = /^javascript:/,
		...
		emailBodyRegex = /body=([^;?:@&=$,\/]*)/,
		anchorRegex = /^#(?!LINKTO:)(.*)$/,
		urlRegex = /^((?:http|https|ftp|news|#LINKTO):\/\/)?(.*)$/,
	...
	var parseLink = function( editor, element ) {
			...
			if ( element ) {
				...
				advAttr( 'advRel', 'rel' );
				retval.contents = element.getHtml();
			}
	...
	return {
			...
			elements: [
				...
				//	validate : CKEDITOR.dialog.validate.notEmpty( 'The Displayed Text field cannot be empty.' ),
					...
					setup: function (data) {
						...
						if (data.contents)		this.setValue(data.contents);
						else 					this.setValue(editor.getSelection().getSelectedText());
					...
					
					{
						...
						[
							...
							},
							{
								type : 'button',
								 id : 'browseInternal',
								 label : commonLang.browsePages,
								 filebrowser :
								 {
									 action : 'Browse',
									 url : 'alleseiten.php?PHPSESSID='+$('#PHPSESSID').val()
								 }
							 }
						]
					}
		...
		onOk: function() {
			...
			if ( !this._.selectedElement ) {
				...
				if (data.contents != '' && range.collapsed) {
					var text = new CKEDITOR.dom.text(data.contents, editor.document);	
					range.insertNode( text );
					range.selectNodeContents( text );
				} else if (range.collapsed ) {			// Use link URL as text with a collapsed cursor.
				...
				if (data.contents)	var textView = data.contents;
				else				var textView = element.getHtml();
				...
				if ( href == textView || data.type == 'email' && textView.indexOf( '@' ) != -1 ) {
					// Short mailto link text view (#5736).
					textView = data.type == 'email' ? data.email.address : attributes[ 'data-cke-saved-href' ];
