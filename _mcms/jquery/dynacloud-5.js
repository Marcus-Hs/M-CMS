 ...
 function innerHighlight(node, pat) {
  ...
  if (node.nodeType == 3) {
   ...
   if (pos >= 0) {
	...
	if (!document.getElementById(pat)) 	spannode.id = pat;
  ...
  if (document.getElementById(pat)) 	{window.location = "#"+pat;}
  return skip;
 }
...
// Adapted from <http://www.perseus.tufts.edu/Texts/engstop.html>
 ...
 stopwords: [ "a", "about", "above", "accordingly", "after",
  ...
  "yes", "your", "yours", "yourself", "yourselves","linkto" ]
...
jQuery.fn.dynaCloud = function(outElement) {
 ...
 return this.each(function() {
  ...
  for (var i = 0; i < l; ++i) {
   out.append('<a title="'+cl[i].el+' - '+(Math.round(100*cl[i].count/k))+' %; '+cl[i].count+'/'+k+'" href="#' + cl[i].el + '" onclick="fckfind(\'' + cl[i].el + '\');" style="font-size: ' + (1+.5*Math.round(cl[i].count/k*10)) + 'em"><span>' + cl[i].el + '</span></a> ');
  }
